-- 020 — agrega 'byUser' a admin_zoho_health: consultas de Zoho POR ASESOR
-- (para el control de uso del agente por cada asesor en /admin/zoho → Salud).
...
as $$
  ...
  select json_build_object(
    ...
    'byUser', (select coalesce(json_agg(u),'[]'::json) from (
        select coalesce(user_email,'(desconocido)') as user_email, count(*) as calls,
               sum((not ok)::int) as errors, coalesce(round(avg(duration_ms)),0) as avg_ms
        from recent group by user_email order by count(*) desc limit 50
      ) u),
